...
extend(Device.prototype, {
  ...
  initPeripheral: function (peripheral, callback) {
    ...
    if (Array.isArray(serviceUuids) && serviceUuids[0]) {
      this.serviceUuid = serviceUuids[0];
    }
  ...
  initListening: function (callback) {
    ...
    this.peripheral.connect(function(error) {
      self.peripheral.discoverServices([], function(error, services) {
        ...
        var serviceInfo;
        var service = getProperService(services,
          this.serviceUuid, services[HR_INDEX]);
...
function getProperService(services, uuid, def) {
  ...
  for (var i = 0, len = services.length; i < len; i++) {
    if (services[i] && services[i].uuid === uuid) {
      return services[i];
    }
  }
  return def;
